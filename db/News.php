...
class News
{
    ...
    {
        if ($arr == NULL) {
            $query = DB::query()->query('SELECT * FROM test_task.news order by id desc');
    ...
    public function get(int $limit, int $offset)
    {
        ...
        $offset *= $limit;
        $query = "SELECT * FROM test_task.news LIMIT $limit offset $offset";
        $query_r = DB::query()->prepare($query);
        $query_r->execute();
    ...
    {
        $sql = "select * from test_task.news order by sort_date desc";
        $sth = DB::query()->prepare($sql);
        $sth->execute();
